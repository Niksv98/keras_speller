...
EOS = '*' # end of sequence.
CHARS = list('aābcčdeēfgģhiījkķlļmnņoprsštuūvzžAĀBCČDEĒFGĢHIĪJKĶLĻMNŅOPRSŠTUŪVZŽ ')
REMOVE_CHARS = '[1234567890#$%"\+@<=>!&,-.?:;()*\[\]^_`•°{|}~/\d\t\n\r\x0b\x0c]'
...
# Funkcija, kas atbildīga par faila nolasīšanu
def read_text(data_path, list_of_books):
    text = ''
    for book in list_of_books:
        file_path = os.path.join(data_path, book)
        strings = open(file_path,"r", encoding="utf8").read()
        text += strings + ' '
    return text
...
def transform(tokens, maxlen, error_rate=0.3, shuffle=True):
    ...
    target_tokens = []
    for token in tokens:
        #Zemak esoso rindu jaaizkomente kad grib izmantot evaluate.py
